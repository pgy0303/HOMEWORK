...
1. FROM 테이블명
2. WHERE 컬럼명 | 단일행함수식 연산자 비교값 | 단일행 함수식
3. GROUP BY 컬럼명 | 함수식
4. HAVING 그룹함수 비교연산 비교값
6. ORDER BY 컬럼명 | 별칭 | 컬럼 순번 정렬방식 [NULLS LAST | FIRST];
*/
...
-- GROUP BY 절 ****************************
-- 같은 값들이 여러 개 기록된 컬럼을 기준으로
-- 같은 값들을 그룹 묶을때 사용함
-- GROUP BY 컬럼명 | 계산식
-- 같은 항목끼리 묶어서 계산할 대 사용함
...
-- GROUP BY 절에 명시하지 않은 컬럼은 SELECT 절에 사용 못함
-- SELECT 절에 그룹함수와 같이 사용할 컬럼은 반드시 
-- GROUP BY 절에 사용되어야 함
-- 부서별, 직급별 급여 합계 , 평균, 직원수 조회
...
SELECT DEPT_ID, SUM(SALARY), FLOOR(AVG(SALARY)), MIN(SALARY), MAX(SALARY)
FROM EMPLOYEE
